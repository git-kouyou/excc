...
\section{発展課題1}
\subsection{実装方針・アルゴリズム}
まず初期値が\texttt{NUMPROCS}であるセマフォを作成し，\texttt{fork()}を用いて\texttt{NUMPROCS}個の子プロセスを生成する．子は5秒以内のランダム秒だけ\texttt{sleep()}してからセマフォを1減算し，メッセージを出力する．すべての子プロセスと親プロセスがセマフォが0になるのを待機し，セマフォが0になるとすべてのプロセスがメッセージを出力して終了する．
\subsection{実行結果}
